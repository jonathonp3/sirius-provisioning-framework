...
The Secure Handoff
Step	Who	What
1	User	Builds artifact in isolated container
2	User	Writes to /run/user/1000/cache/ (user-owned)
3	Root	Systemd path unit triggers
4	Root	Reads and deploys artifact
5	Root	Cleans up trigger file
Why It's Secure
Aspect	Security Benefit
User has no sudo	User can't modify system directories
Root only reads	Root doesn't execute user scripts
Atomic handoff	No race conditions or corruption
Event-driven	No polling, immediate trigger
